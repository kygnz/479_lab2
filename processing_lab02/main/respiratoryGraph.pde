...
void drawRespGraph(){
    pushMatrix();
      ...
      float maxY = maxRespRate;  // Maximum Y-axis value (max heart rate)
    ...
    //    float y1 = map(respRateY.get(i - 1), minY, maxY, chartY + chartHeight - 40, chartY);
    //    float x2 = map(respTimeX.get(i), respTimeX.get(0), respTimeX.get(respTimeX.size() - 1), chartX + 40, chartX + chartWidth - 20);
    //    float y2 = map(respRateY.get(i), minY, maxY, chartY + chartHeight - 40, chartY);
